package dev.khanhtimn.khuacraft.sound;

import dev.khanhtimn.khuacraft.KhuaCraft;
import dev.khanhtimn.khuacraft.sound.sounds.InnerConscienceSound;
 ...
 */
@EventBusSubscriber(modid = KhuaCraft.MODID, value = Dist.CLIENT)
public class ModSoundHandler {

    public static final Map<UUID, PlayerSound> innerConscienceSounds = new Object2ObjectOpenHashMap<>();
    ...
    public static void restartSounds() {
        ...
        if (hasPlayerSounds != hadPlayerSounds) {
            ...
            if (hasPlayerSounds) {
                ...
                innerConscienceSounds.values().forEach(ModSoundHandler::restartSounds);
            }
        }
    }
    ...
    public static void playSound(SoundEvent sound) {
        playSound(SimpleSoundInstance.forUI(sound, 1.0F, 1.0F));
    }
    ...
    public static void onTilePlaySound(PlaySoundEvent event) {
        ...
        if (!soundLoc.getNamespace().startsWith(KhuaCraft.MODID)) {
            return;
        }
    ...
    private static class TileTickableSound extends AbstractTickableSoundInstance {
        ...
        public void tick() {
            ...
            assert level != null;
            if (level.getGameTime() % checkInterval == 0) {
